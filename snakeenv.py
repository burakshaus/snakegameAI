...
def updating_snake(SnakeDir, SnakeList, SnakeEat, SnakeDead):
    if not SnakeDead:
        if not SnakeEat:
            SnakeList.pop(-1)  # Remove tail unless the snake has eaten
        else:
            SnakeEat = False  # Reset eating flag
...
def collision(SnakeList, ApplePos, SnakeDir, SnakeEat, SnakeDead, Score):
    # apple and snake head collision
    if SnakeList[0] == ApplePos:
        SnakeEat = True
        Score += 1
        ApplePos = []  # Clear apple position to generate new one

    # snake head and wall collision
...
class SnakeEnv(gym.Env):
    ...
    def step(self, action):
        self.snake_eat, self.snake_dead, self.score, self.apple_pos = collision(self.snake_list, self.apple_pos, self.snake_dir, self.snake_eat, self.snake_dead,
                                                            self.score)
    ...
    def reset(self):
        ...
        if self.snake_dir == 'up':
            ...
            self.numerical_dir = -1  # Default value if no direction is set
        ...
        if self.render_mode == 'human':
            ...
            self.font = pygame.font.SysFont('Arial', 380)
            self.render()
    ...
    def render(self, render_mode = 'human'):
        ...
        # font = pygame.font.SysFont('Arial', 380)
        ...
        self.display.blit(img, img.get_rect(center=(20 * 15 + 15, 15 * 15 + 15)))
